...
Источник истины — **только `.mpp`**. Справочник задач / ВОР / ВОР_факт читаются через MS Project COM. На Mac локальный `POST /api/mpp/upload` без COM **проксирует файл на Windows API** и подставляет `options.tasks`. Без живого хоста с Project форма не разблокируется (в селекте не будет пустого «нет значений»).
...
3. Заполнить план/факт по неделям — «План на месяц», «Факт за месяц» и отклонение считаются сами (сумма недель).
