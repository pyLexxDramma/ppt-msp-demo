...
На Streamlit Cloud `.mpp` выбирается **нативным `st.file_uploader`** (блок «1. Исходный .mpp» над React-формой) — так надёжно уходят крупные файлы. В Vite/локальном SPA загрузка остаётся внутри React-карточки через `/api/mpp/upload`.
