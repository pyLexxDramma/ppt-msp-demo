...
На Streamlit файл выбирается **в пункте 1 формы** (как в Vite): байты держатся в браузере до пересчёта, затем уходят через Windows-туннель или одним сообщением с recalc (лимит ~1.5 МБ без туннеля).
